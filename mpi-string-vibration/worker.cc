...
#pragma omp parallel for simd
    ...
    MPI_Status status;
    int r = rank+1, l = rank-1;
    if(rank == 0){
      MPI_Sendrecv(&d_t[last_segment-1], 1, MPI_FLOAT, r, 0, &d_t[last_segment], 1, MPI_FLOAT, r, 0, MPI_COMM_WORLD, &status);
    }else if(rank == world_size-1){
      MPI_Sendrecv(&d_t[start_segment], 1, MPI_FLOAT, l, 0, &d_t[start_segment-1], 1, MPI_FLOAT, l, 0, MPI_COMM_WORLD, &status);
    }else{
      MPI_Sendrecv(&d_t[last_segment-1], 1, MPI_FLOAT, r, 0, &d_t[start_segment-1], 1, MPI_FLOAT, l, 0, MPI_COMM_WORLD, &status);
      MPI_Sendrecv(&d_t[start_segment], 1, MPI_FLOAT, l, 0, &d_t[last_segment], 1, MPI_FLOAT, r, 0, MPI_COMM_WORLD, &status);
    }
  }
