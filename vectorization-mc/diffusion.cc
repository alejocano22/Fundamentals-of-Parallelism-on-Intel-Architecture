...
int diffusion(const int n_particles, 
  ...
  // Temporary buffer to store the positions of the particles.
  float x[n_particles];
  x[0:n_particles]=0.0f;

  // Generate and store multiple random numbers before the vectorized loop.
  ...
  // Implement loop-interchange to make the particle loop to the inner loop.
